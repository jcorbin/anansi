...
func (sd *schotterDemo) draw() {
	for y := 0; y < sd.squaresPerCol; y++ {
		for x := 0; x < sd.squaresPerRow; x++ {
			...
			if y > 1 {
				r1 := rand.Float64() / float64(sd.squaresPerCol) * float64(y)
				r2 := rand.Float64() / float64(sd.squaresPerCol) * float64(y)
				r3 := rand.Float64() / float64(sd.squaresPerCol) * float64(y)
				if rand.Intn(2) == 1 {
					r1 = -r1
				}
				if rand.Intn(2) == 1 {
